...
class RoutingSpec extends WordSpec with MustMatchers {
  ...
  "Routing" must {
    ...
    "dispatch" in {
      ...
      val timeout = (5 seconds).dilated.toMillis
      val result = for {
        a ← (d !! (Test1, timeout)).as[Int]
        b ← (d !! (Test2, timeout)).as[Int]
        c ← (d !! (Test3, timeout)).as[Int]
      } yield a + b + c
    ...
    "dispatch to smallest mailbox" in {
      ...
      try {
        ...
        case e: TestLatchTimeoutException ⇒
          throw new TestLatchTimeoutException(e.getMessage + " t1=" + t1Count + " t2=" + t2Count)
      }
